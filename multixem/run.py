...
def bootstrap_mean_map(refined_mtzs, idx=0, prefix=""):
    ...
    def merge_reflections_bootstrap(
        ...
        """
        f_complex_acentric = (
            acentric.groupby(["H", "K", "L"])["F_complex"]
            .agg(
                [
                    ("F_complex_mean", lambda x: numpy.mean(x)),
                    (
                        ...
                        lambda x: (
                            numpy.sqrt(
                                (numpy.var(numpy.real(x)) + numpy.var(numpy.imag(x)))
                                / 2
                            )
                            if len(x) > 1
                            else 0
        ...
        f_complex_centric = (
            centric.groupby(["H", "K", "L"])["F_complex"]
            .agg(
                [
                    ("F_complex_mean", lambda x: numpy.mean(x)),
                    (
                        "SIGFWT",
                        lambda x: (
                            numpy.sqrt(numpy.var(numpy.absolute(x)))
                            if len(x) > 1
                            else 0
                        ),
                    ),
        ...
        del_f_complex_acentric = (
            acentric.groupby(["H", "K", "L"])["DEL_F_complex"]
            .agg(
                [
                    ("DEL_F_complex_mean", lambda x: numpy.mean(x)),
                    (
        ...
        del_f_complex_centric = (
            centric.groupby(["H", "K", "L"])["DEL_F_complex"]
            .agg(
                [
                    ("DEL_F_complex_mean", lambda x: numpy.mean(x)),
                    (
        ...
        # Dataframe with acentric and centric reflections together
        f_complex_mean = pandas.concat(
            [f_complex_acentric, f_complex_centric], ignore_index=True
        )
        del_f_complex_mean = pandas.concat(
            [del_f_complex_acentric, del_f_complex_centric], ignore_index=True
        )
        df_mean = f_complex_mean.merge(
            del_f_complex_mean, on=["H", "K", "L"], how="outer"
        )
        """
