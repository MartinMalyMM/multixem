...
def create_parser():
    ...
    parser.add_argument('-u', '--hklin_unmerged', type=str,
                        help="Input unmerged diffraction data.") ## TODO - file exists?
    parser.add_argument('--n_batches', type=positive_int, default=60,
...
def merge_in_groups(unmerged, n_batches_in_group, prefix):
    ...
    def merge_group(df_groups, i_group, cell, spacegroup, n_expected, wavelength=0, n_groups=0, prefix=''):
        intensities = gemmi.Intensities()
        intensities.set_data(cell, spacegroup,
        ...
        print(f"Merged group {i_group+1} of batches: #reflections:",
        ...
        mtz_group_merged.write_to_file(
            f"{prefix}group{g_with_leading_zeros}.mtz")  ## TODO
    ...
    try:
        if len(batch.floats) > 37 and batch.floats[36] and batch.floats[37]:
            print(f"Start/end phi of the first batch found: {batch.floats[36]}, {batch.floats[37]}")
        else:
    ...
    # Note that batch numberring is 1, 2, ..., 2000 but Python numberring is 0, 1, ..., 1999
    ...
    for i_group in range(len(batches_split)-1):
        ...
        df_group = df.loc[(df['BATCH'] >= batches_split[i_group]) & (df['BATCH'] < batches_split[i_group+1])]
        df_groups.append(df_group)
        merge_group(df_groups, i_group,
                    m.cell, m.spacegroup, n_expected, wavelength,
                    n_groups=len(batches_split), prefix=prefix)
